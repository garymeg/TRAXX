...
30      $1E     BREAK (located in 50020 $C364 table)
        */
        ...
        .label      LODARG           = $DA8C // Move floating point memory locations to FAC2. .A and .Y point to the four bytes of memory that contain a three-byte mantissa, plus a sign byte.        Decimal >       55948
        .label      MULDIV           = $DAB7 // Add exponents of FAC and FAC2. This routine stores the sum of FAC and FAC2 exponents in FAC exponent, testing for an OVERFLOW error. 59034 SDAE2 MULTEN
        .label      MULTEN           = $DAE2 // Multiply FAC by 10. MULTEN is a subroutine called when converting floating point to an ASCII value, for example TI$. The PLUS routine 55402 ($D86A) is called to do the task.    
        .label      FPCTEN           = $DAF9 // +10 floating point constant: $84,20,00,00,00.        Decimal >       56057
        .label      DIVTEN           = $DAFE // Divide FAC by 10. The routine DIVIDE at location 56082 ($DB12) is called to do the work.        Decimal >       56062
        .label      LADIV            = $DB0F // Move floating point in memory to FAC2. In preparation for division, this loads FAC2 from memory location.        Decimal >       56079
        .label      DIVIDE           = $DB12 // BASIC / (Divide FAC2 by FAC resulting in FAC). Before performing the divide operation, a check for DIVISION BY ZERO is made.
        .label      LODFAC           = $DBA2 // Move floating point memory into FAC. This is called by other routines to load a floating point number pointed to by .A and .Y into FAC.        Decimal >       56226
        .label      FACTF2           = $DBC7 // Move FAC to memory. This routine is called to store a floating point number into memory location 92-96 ($5C-60), part of TEMPF3.
        .label      FACTF1           = $DBCA // Move FAC to memory. FACTFl is called to store a floating point number into memory location 87-92 ($57-5B), part of TEMPF3. The second byte of the value to be loaded is ORed with 128 ($80) when loading the Floating Point Accumulator.
        .label      FACTFP           = $DBD0 // Move FAC to memory. I This is called to store a floating point number into memory, at the location pointed to by 73-74 ($49-4A).
        .label      STORFAC          = $DBD4 // Perform move of FAC to memory. The above routines fall through to this routine to actually if accomplish the storing of FAC to memory, now pointed to by .X and .Y. This routine uses the first two bytes of location 34-37 ($22-25) as a pointer to the target location.
        .label      ATOF             = $DBFC // Transfer FAC2 to FAC. This is a loop that transfers five bytes from FAC2 to FAC; the sixth byte (sign) is forced to be positive by storing a zero in it.
        .label      RFTOA            = $DC0C // Move FAC to FAC2, with rounding. This calls the routine ROUND 56347 ($DC1B) and then falls through.
        .label      FTOA             = $DC0F // Move FAC to FAC2, without rounding. FTOA moves six bytes (the full exponent, mantissa, and sign) of FAC to FAC2.
        .label      ROUND            = $DC1B // Round FAC by adjusting the rounding byte. Location 112 ($70) is doubled, and if now greater than 128 ($80), a one is added to the FAC.
        .label      SGNFAC           = $DC2B // Test the sign of FAC. On exit, .A=0 if zero; 1 if positive; or 255 ($FF) if negative.
        .label      SGN              = $DC39 // BASIC SGN. This calls the routine SGNFAC 56363 ($DC2B) then falls through.
        .label      INTFP            = $DC3C // Convert the sign obtained above to 0 or —1 in FAC. If entered without falling through from SGN 56377 ($DC39), this routine converts .A to floating point in FAC.
        .label      INTFP1           = $DC44 // Convert a two-byte integer to floating point in FAC. The number in location 98-99 ($62-63) is converted into FAC.
        .label      ABS              = $DC58 // BASIC ABS. The FAC sign byte at 102 ($66) is shifted right one bit to remove any negative sign. The high order bit equals 1 when negative; 0 when positive.
        .label      CMPFAC           = $DC5B // Compare FAC to memory. .A and .Y point to the five-byte memory location to be compared with FAC. Afterward, .A=0 if equal; 1 if the memory location is less than FAC; and 255 ($FF) if the memory location contains a number greater than FAC.
        .label      FPINT            = $DC9B // Convert FAC floating point to signed integer. The resulting four-byte integer is left in 98-101 ($62-65). Routine INTFPl 56388 ($DC44) converts a two-byte integer into floating point.
        .label      INT              = $DCCC // BASIC INT. FAC is rounded down to an integer in floating point format. The result is four bytes in locations 98-101 ($62-65). This routine is called by many functions requiring an integer in floating point format.
        .label      FILFAC           = $DCE9 // Store the contents of .A in locations 98-101 ($62-65). Used to zero out the locations.
        .label      ASCFLT           = $DCF3 // Convert an ASCII string to a floating point number in FAC. This is called by VAL to evaluate and convert the string, allowing +, —, E, spaces, and a decimal point in the string. Note that location 95 ($5F) is used as a switch to indicate that a decimal point I I has already been processed.
        .label      ASC18            = $DD7E // Add .A to FAC. Part of the ASCFLT routine at 56563 ($DCF3), this adds .A to p. FAC by calling other routines.
        .label      FPC12            = $DDB3 // String to floating point conversion constants.
        .label      PRTIN            = $DDC2 // Issue message IN. .A and .X are loaded with the contents of location 57-58 ($39-3A), then fall through to the next routine.
        .label      PRTFIX           = $DDCD // Decimal number display routine. An integer number (.A*256+.X) is converted to floating point in FAC. The routine FLTASC 56797 ($DDDD) is called to convert the number to a string, and the routine PRTSTR 51998 ($CB1E) is called to print the number. This routine can be called from a machine lan guage program.
        .label      FLTASC           = $DDDD // Convert FAC to TI$ or an ASCII string. A fairly long and involved routine. A work area at 256-270 ($100-10E) is used and the result is left in that area.
        .label      FLP05            = $DF11 // 0.5 constant for rounding and SQR. $80,00,00,00,00
        .label      FLTCON           = $DF16 // Powers of 10 table, in four-byte fixed integer format. I This is used for converting strings, Constant Fixed Integer Representation, -100,000,000 $FA,OA,1F,OO, 1 +10,000,000 $00,98,96,80, -1,000,000 $FF,F0,BD,C0, + 100,000 $00,01,86,A0, -10,000 $FF,FF,D8,F0, + 1,000 $00,00,03,E8, -100 $FF,FF,FF,9C, + 10 $00,00,00,0A, -1 $FF,FF,FF,FF
        .label      HMSCON           = $DF3A // Constants for TI$ division conversion, in four-byte fixed integer format.        Decimal >       57146
        .label      SQR              = $DF71 // BASIC SQR.I This moves FAC to FAC2, loads FAC with 0.5, and falls through.
        .label      EXPONT           = $DF7B // ASIC t (up arrow/power). This routine calculates FAC2 to the FAC power, resulting in FAC. It calls LOG 55786 ($D9EA) for FAC2, which is multiplied by FAC and calls EXP 57325 ($DFED).        Decimal >       57211
        .label      NEGFAC           = $DFB4 // BASIC monadic- Negate FAC by exclusive ORing the sign byte with a constant of 255 ($FF). Zero is left unchanged.$DFBF        Decimal >       57268
        .label      EXPCON           = $DFBF // Table for EXP, in floating point format. Used to calculate 2 to the N power.        Decimal >       57279
        .label      EXP              = $DFED // BASIC EXP. The value in FAC is multiplied by 1/LOG(2), converted to an integer, and the table at 57279 ($DFBF) is applied to calculate e (2.718281828) to the power of FAC, and left in FAC. EXP is the reverse function of LOG. For EXP(X), the same results can be achieved by 2.7182818 t X.        Decimal >       57325
        .label      SEREVL           = $E040 // Series evaluation subroutine.This calls the next routine to accomplish the evaluation of complex trigonometric functions. A pointer is passed to this routine. This pointer is stored in location 113-114 ($71-72) and is known as the series evaluation pointer. It points to the table of constants for the trigonometric function being evaluated by the evaluation routines. This pointer will specify a location within the tables at 58171 ($E33B), 55745 ($D9C1), 57284 ($DFC4), or 58092 ($E2EC).        Decimal >       57430
        .label      SER2             = $E056 // Math series evaluation routine. This routine computes polynomials based upon the table pointer passed to it. This table pointer is stored in location 113-114 ($71-72). The initial pointer passed to this routine points at the number of table entries to be processed, which is followed by the table values. SER2 multiplies and adds the coefficients to FAC to complete the calculation.        Decimal >       57482
        .label      RNDX             = $E08A // Table of constants for RND. This contains two floating point format numbers to use when the argument passed to the BASIC command RND is a positive number.        Decimal >       57492
        .label      RND              = $E094 // BASIC RND. See the discussions at location 139 ($8B) and 57482 ($E08A) for details of the RND function.        Decimal >       57590
        .label      PATCHBAS         = $E0F6 // BASIC patch routines. These routines are apparently inserted here to invoke CLR when j j RS-232 is opened, and to display an error message if a called Kernal routine returns with the carry bit set. BREAK is assumed if the carry bit is set, but the error message number is zero.        Decimal >       57639
        .label      SYSTEM           = $E127 // BASIC SYS. The argument expression is evaluated, then type checked and converted to a two-byte integer format by calling routine MAKADR 55287 ($D7F7), which stores the result in LSB/MSB format at location 20-21 ($14-15). A return address within the SYS routine is j 1 pushed onto the stack. Then the registers at locations 780-783 '—l ($30C-30F) are loaded and an indirect JMP is performed off the vec tor stored in location 20-21 ($14-15). When SYS is reentered by the r , RTS of the target routine, the registers are stored in the SAVE area I I of 780-783 ($30C-30F) for the next SYS to use.        Decimal >       57683
        .label      BSAVE            = $E153 // BASIC SAVE. The routine calls PARSL 57809 ($E1D1) to set the filename, device, and secondary address parameters. .X and .Y are loaded with the contents of locations 45-46 ($2D-2E), which is the pointer to the end of the BASIC program, and .A is loaded with the constant of 43 ($2B), the zero page location of the pointer to the start of the BASIC program. A JMP is then made to the Kernal SAVE routine via the vector at $FFD8.          Decimal >       57698
        .label      BVERIF           = $E162 // BASIC VERIFY. This sets .A to 1 to indicate a VERIFY is in progress and falls through to the next routine, skipping one instruction.        Decimal >       57701
        .label      BLOAD            = $E165 // BASIC LOAD. The first instruction of this routine loads .A with a zero to indicate that a LOAD was requested. Note that this is skipped by BVERIF.        Decimal >       57787
        .label      FOPEN            = $E1BB // BASIC OPEN. The routine PAROC 57878 ($E216) is called to set the filename, l—' device, secondary address, and argument expression that may be specified with OPEN. See that routine for more details of the OPEN command. '—' This JMPs off the COPEN* vector at $FFC0 to $031A and finally to the Kernal routine OPEN at 62474 ($F40A).
        .label      FCLOSE           = $E1C4 // BASIC CLOSE. The routine PAROC at 57878 ($E216) is called to set the file number. This then JMPs off the CCLOS vector at 65475 ($FFC3) to $031C, and finally to the Kernal routine CLOSE 62282 ($F34A). If closing tape, an end of file byte (0) is written, and perhaps an I.D. 5 header, if opened with a secondary address of 2, to indicate end of tape.        Decimal >       57796
        .label      PARSL            = $E1D1 // Set LOAD, VERIFY, and SAVE parameters. This is a rather curious routine that first calls the Kernal routine SETNAM 65097 ($FE49), assuming that there is no filename, and causes the device number and secondary address to default to 1 and 0 by calling the Kernal routine SETLFS 65104 ($FE50) with these parameters set.        Decimal >       57809
        .label      IFCHRG           = $E203 // Check whether more characters are in the current statement. If a call to CHRGET indicates that more characters are in this statement, then return. Otherwise, return to the routine that called the calling routine.        Decimal >       57859
        .label      SKPCOM           = $E20B // Skip any comma in parameters being scanned. This routine calls the routine COMCHK 52989 ($CEFD) to do its work, then falls through to the next routine.        Decimal >       57867
        .label      CHRERR           = $E20E // Insure that a parameter is present after a delimiting comma. If a call to CHRGET indicates that more characters are in this statement, then return. Otherwise, issue a SYNTAX ERROR message.        Decimal >       57870
        .label      PAROC            = $E216 // Handle parameters for OPEN and CLOSE. This routine sets the filename, device, secondary address, and argument expression that may be specified with OPEN or CLOSE. The routine first calls the Kernal routine SETNAM 65097 ($FE49), assuming that there is no filename, and causes the device number and secondary address to default to 1 and 0 by calling the Kernal routine SETFLS 65104 ($FE50) with these parameters set.        Decimal >       57878
        .label      COS              = $E261 // BASIC COS. The cosine of FAC in radians is placed in FAC. This function can also be performed with SIN(X+(?/2)). This adds a constant value of ?/2 to the value and falls through to the next routine.        Decimal >       57953
        .label      SIN              = $E268 // BASIC SIN. Calculates the sine of FAC in radians. This is also used, with        Decimal >       57960
        .label      TAN              = $E2B1 //  BASIC TAN. Determines the tangent of FAC in radians. This routine calls SIN 57960 ($E268) and COS 57953 ($E261) to help evaluate the expression. SE2DD       Decimal >       58033
        .label      FPC20            = $E2DD //  Trigonometric evaluation constant values used for COS, SIN, and TAN. These table values are in five-byte floating point format.       Decimal >       58077
        .label      ATN              = $E30B // BASIC ATN. Determines the arctangent of FAC in radians. Simpler than other trigonometric functions, this applies the rather long table of constant values at ATNCON 58171 ($E33B).        Decimal >       58123
        .label      ATNCON           = $E33B // Table of constant values for ATN evaluation. The values are in five-byte, floating point format.        Decimal >       58171
        .label      COLDBA           = $E378 // Perform a cold start of BASIC. This routine is pointed to by the vector stored in location 49152 ($C000) COLDST*, that is branched upon as the last act of the Kernal during its power-on/reset routine START 64802 ($FD22), which the 6502 automatically jumps to via the vector stored in location 65532 ($FFFC).        Decimal >       58232
        .label      CGIMAG           = $E387 // CHRGET routine and RND seed to be copied to page zero RAM. The CHRGET routine stored here is copied to locations 115-138 ($73-$8A) and the seed value for RND is copied to locations 139-143 ($8B-8F). This seed value is .811635157 ($80,4F,C7,52,58).        Decimal >       58247
        .label      INITBA           = $E34A // Initialize BASIC: Restore CHRGET and page zero pointers. This routine is called by COLDBA* 58232 ($E378) during the cold start of BASIC.        Decimal >       58276
        .label      FREMSG           = $E404 // Display cold start of BASIC messages. This is called by COLDBA* 58232 ($E378) during the cold start of BASIC.        Decimal >       58372
        .label      CBMMSG           = $E429 // BASIC cold start messages. The **** CBM BASIC V2 **** and BYTES FREE messages.       Decimal >       58409
        .label      BASVCTRS         = $E44F // Six BASIC vectors to be copied to location 768 ($300). The INITVCTRS* routine at 58459 ($E45B) copies these vectors to RAM. See the description of these vectors at their RAM locations 768-778 ($300-$30A).        Decimal >       58447
        .label      INITVCTRS        = $E45B // Copy BASIC vectors from ROM to RAM. This is called by COLDBA* 58232 ($E378) during the cold start of BASIC. Six BASIC vectors at location 58447 ($E44F) are copied to RAM. See the description of these vectors at their RAM locations 768-778 ($300-$30A).        Decimal >       58459
        .label      WARMBAS          = $E467 // Perform a warm start of BASIC. When both the RUN/STOP and RESTORE keys are pressed, the BREAK* routine at 65234 ($FED2) uses the vector at ($C002) to branch here after it has completed its duties.        Decimal >       58471
        .label      PATCHER          = $E476 // Program patch area. This contains two instructions that are used by the routine BLOAD 57701 ($E165) to add calls which rechain the program lines by calling routine LNKPRG 50483 ($C533), to call the routine RESTOR at 51229 ($C81D), and to reset the temporary string stack pointer and the 6502 stack pointer.        Decimal >       58486
    }       
KERNAL: //Kernal Pointers and routines    
    {       
        .label      SEROUT1          = $E4A0 //  Serial: output a 1 on the serial data line.       
        .label      SEROUTO          = $E4A9 //  Serial: output a 0 on the serial data line.       
        .label      SERGET           = $E4B2 //  Serial: get an input bit from VIA1 and stabilize.       
        .label      PATCHES          = $E4BC //         
        .label      IOBASE           = $E500 //         
        .label      SCRN             = $E505 //         
        .label      PLOT             = $E50A //         
        ...
        .label      ISCNTC           = $FFE1 // 
        .label      CGETL            = $FFE4 // 
        .label      CCALL            = $FFE7 // JuMP off 812-813 ($32C-32D) ICLALL.
        .label      CUDTIM           = $FFEA // JuMP to 63284 ($F734) UDTIM.
        .label      CSCREEN          = $FFED // JuMP to 58629 ($E505) SCRN.
        .label      CPLOT            = $FFF0 // JuMP to 58634 ($E50A) PLOT.
        .label      CIOBASE          = $FFF3 // JuMP to 58624 ($E500) IOBASE.
        .label      VCTRNMI          = $FFFA // 6502 vector to 65193 ($FEA9) NMI*.
        .label      VCTRRST          = $FFFC // 6502 vector to 64802 ($FD22) START.  
        .label      VCTRIRQ          = $FFFE // 6502 vector to 6      
    }        
